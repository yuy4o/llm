...
# default
DEFAULT_CKPT_PATH = "/data1/jiangyy/imagemodels/Qwen2-VL-2B-Instruct"
VIDEO_EXTENSIONS = [
    ".mp4",
    ".avi",
    ".mkv",
...
def _get_args() -> Namespace:
    ...
    parser.add_argument(
        "--port", type=int, default=12000, help="Demo服务器端口, 默认为`12000`。"
    )
    parser.add_argument(
        "--host", default="0.0.0.0", help="DDemo服务器地址, 默认为`127.0.0.1`。"
    )
...
class LazyModelLoader:
        ...
        with self.lock:
            if self.model is None:  # 确保模型只加载一次
                ...
                try:
                    ...
                    # 检查是否使用 GPU 1（编号会变成 0）
                    print("GPU Name:", torch.cuda.get_device_name(torch.cuda.current_device()))
                    model, proc = self._load_model_processor(device)
    ...
    def _load_model_processor(
        ...
        """
        args = self.args
        device = torch.device("cuda:0")  # 设置设备为 GPU1
        device_map = None  # 避免自动映射，手动设置设备
...
def _pred(
    ...
    gen_config = GenerationConfig(max_new_tokens=4096, **_gen_kwargs)
...
# python mm_qwen2vl.py --model-path Qwen2-VL-2B-Instruct --dtype bf16