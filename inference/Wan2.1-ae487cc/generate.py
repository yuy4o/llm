...
def generate(args):
    ...
    if rank == 0:
        ...
        if "t2i" in args.task:
            ...
            # video: [T, C, H, W] 或 [1, T, C, H, W]
            vid = video
            
            vid = vid.detach().cpu()

            # ⚠️ 注意：不要再动 channel / time
            # [C, T, H, W] → [T, C, H, W]
            vid = vid.permute(1, 0, 2, 3)

            # 如果是 latent，只在「channel 维」裁剪
            if vid.shape[1] > 3:
                vid = vid[:, :3, :, :]   # ✅ 现在这是 C 维
            ...
            # [T, C, H, W] → [T, H, W, C]
            frames = vid.permute(0, 2, 3, 1).contiguous().numpy()
            
            print("raw video shape:", video.shape)
            print("frames shape:", frames.shape)

            assert frames.ndim == 4
            assert frames.shape[-1] in (1, 3, 4), frames.shape
