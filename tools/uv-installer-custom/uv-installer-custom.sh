...
_UV_DOWNLOAD_PROXY_URL="${UV_DOWNLOAD_PROXY:-https://ghfast.top}"
# --- Customization End ---
if [ -n "${UV_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$UV_INSTALLER_GHE_BASE_URL"
else
    INSTALLER_BASE_URL="${UV_INSTALLER_GITHUB_BASE_URL:-${_UV_DOWNLOAD_PROXY_URL}/https://github.com}"
fi
if [ -n "${UV_DOWNLOAD_URL:-}" ]; then
    ...
    ARTIFACT_DOWNLOAD_URL="${INSTALLER_BASE_URL}/astral-sh/uv/releases/download/0.8.5"
fi
...
_UV_DOWNLOAD_PROXY_URL_FOR_CONFIG="${UV_DOWNLOAD_PROXY:-https://ghfast.top}"
_UV_PYPI_MIRROR_URL="${UV_PYPI_MIRROR:-https://pypi.tuna.tsinghua.edu.cn/simple}"
...
# --- Customization End --- say "everything's installed!"
