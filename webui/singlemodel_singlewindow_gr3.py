...
# from your_module import WebUI  # 假设 WebUI 类已经保存为 your_module.py
...
llm = {
    ...
    # OpenAI-compatible 接口（vLLM）
    "base_url": "http://localhost:12000/v1",
    "api_key": "empty",

    # 生成参数 → 会进入 BaseChatModel.generate_cfg
    "generate_cfg": {
        "temperature": 0.7,
        "top_p": 0.9,
        "max_tokens": 2048,
...
class SimpleAgent(Agent):
    def __init__(self, llm):
        super().__init__(
            name="SimpleBot",
            description="A local model powered agent",
            llm=llm,
        )

    def _run(self, messages, **kwargs):
        """
        messages: List[Message]，已经是 qwen-agent 标准格式
        """
        kwargs.pop("lang", None)
        for rsp in self.llm.chat(messages, **kwargs):
            yield rsp

def run_app():
    simple_agent = SimpleAgent(llm=llm)
